...
public class ChannelIntercerptorTest {
    @Autowired
    ...
    public void testChannels() throws InterruptedException {
        ...
        Assert.assertEquals(1, this.auditService.getAuditRecords().size());
        AuditRecord auditRecord = this.auditService.getAuditRecords().get(0);
        Assert.assertTrue(auditRecord.getAuditInstance() instanceof ChargedBooking);
        Assert.assertEquals("user@example.com", ((ChargedBooking)auditRecord.getAuditInstance()).getBooking().getCustomerEmail());
    }
}
